...
import { Gavel, Play, Pause, FastForward, SkipForward, AlertCircle, Bot, ArrowRight, XCircle, Coffee } from 'lucide-react';
        ...
        <div className="pointer-events-auto bg-slate-900/90 backdrop-blur-xl border border-slate-600 rounded-2xl shadow-2xl p-4 flex flex-col md:flex-row items-center gap-6 md:gap-12 min-w-[320px] md:min-w-[600px] max-w-4xl w-full relative">
            ...
            {isHost && (
                <div className="absolute -top-16 right-0 bg-slate-900/90 border border-slate-600 p-2 rounded-xl flex gap-2">
                    ...
                    <button onClick={startBreak} className="p-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white flex items-center justify-center gap-2 font-bold text-xs transition-colors" title="Start Strategic Timeout">
                        <Coffee size={14} />
                    </button>
                    {!isRoundComplete && (
      ...
      <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-2">
         <div className="bg-slate-900/90 backdrop-blur border border-slate-600 p-4 rounded-xl shadow-2xl w-64">
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
                ...
                <button onClick={startBreak} className="p-3 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white flex items-center justify-center gap-2 font-bold text-sm transition-colors">
                    <Coffee size={16} /> Break
                </button>
                <button onClick={skipPlayer} className="p-3 rounded-lg bg-red-900/50 border border-red-700 hover:bg-red-900 text-red-200 flex items-center justify-center gap-2 font-bold text-sm transition-colors col-span-2">
