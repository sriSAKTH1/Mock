...
export const AuctionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setsCompletedRef = useRef<number>(0);
        ...
        if (isHost) {
            socket.emit('syncState', {
                ...
                state: {
                    ...
                    isBreak,
                    breakEndTime,
                    setsCompleted: setsCompletedRef.current
                }
            });
        }
    };
  }, [isHost, roomCode, players, teams, currentPlayerId, timer, isPaused, currentBid, isBreak, breakEndTime]);
  ...
  useEffect(() => {
    ...
    const onSyncState = (state: any) => {
        ...
        if (state.setsCompleted !== undefined) setsCompletedRef.current = state.setsCompleted;
        if (state.isBreak !== undefined) setIsBreak(state.isBreak);
        if (state.breakEndTime !== undefined) setBreakEndTime(state.breakEndTime);
    };
    ...
    const onBreakStarted = ({ endTime }: { endTime: number }) => {
    ...
    socket.on("breakEnded", onBreakEnded);
    ...
    return () => {
      ...
      socket.off("breakEnded", onBreakEnded);
    };
  }, []);
  ...
  const startBreak = () => {
    ...
    const endTime = Date.now() + 90000; // 1 min 30 sec
    setBreakEndTime(endTime);
    setIsBreak(true);
    socket.emit("startBreak", { roomCode, endTime });
  };
  ...
  const endBreak = () => {
    ...
    setBreakEndTime(null);
    setIsBreak(false);
    socket.emit("endBreak", { roomCode });
  };

  // Auto-end break for host
  useEffect(() => {
    if (isHost && isBreak && breakEndTime) {
  ...
  const nextPlayer = () => {
    ...
    if (currentP && nextP && currentP.set !== nextP.set) {
       ...
       if (setsCompletedRef.current > 0 && setsCompletedRef.current % 2 === 0) {
           triggerBreak = true;
       }
    }
