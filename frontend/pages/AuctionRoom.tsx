...
import { AuctionSet, Player, Team } from '../types';
import { History, TrendingUp, Crown, Users, LayoutList, Search, User, ArrowRight, List, Wifi, Bot, Layers, X, Gavel, Sparkles, Activity, ArrowLeft, Coffee } from 'lucide-react';
    ...
    <div className="h-screen bg-ipl-dark flex flex-col overflow-hidden relative font-roboto text-slate-200">
            ...
            <div className="flex gap-3">
                {teams.map(team => (
                    <div key={team.id} className={`flex items-center gap-3 px-3 py-1.5 rounded bg-slate-800/80 border ${team.players.length === currentPlayer?.soldPrice ? 'border-ipl-gold' : 'border-slate-700'} min-w-max`}>
                         {team.logoUrl ? (
                             <img src={`${SERVER_ORIGIN}/proxy?url=${encodeURIComponent(team.logoUrl)}`} alt={team.shortName} className="w-8 h-8 object-contain" />
                         ) : (
      ...
      <div className="flex-1 flex overflow-hidden relative z-10">
        ...
        <main className="flex-1 p-4 md:p-8 flex flex-col relative overflow-hidden pb-36">
             {isBreak && (
               <div className="absolute inset-0 z-30 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center">
                 <div className="relative">
                   <div className="absolute -inset-10 bg-ipl-gold/20 rounded-full blur-[100px] animate-pulse"></div>
                   <div className="relative flex flex-col items-center gap-8">
                     <div className="flex items-center gap-4 text-ipl-gold">
                       <Coffee size={64} strokeWidth={1.5} />
                       <h1 className="text-6xl md:text-8xl font-teko font-bold uppercase tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-ipl-gold via-yellow-200 to-ipl-gold">
                         Strategic Timeout
                       </h1>
                     </div>
                     <div className="flex flex-col items-center justify-center p-10 bg-slate-900/60 border border-slate-700/50 rounded-3xl shadow-2xl">
                       <div className="text-7xl md:text-9xl font-mono font-bold text-white tracking-tighter tabular-nums mb-2">
                     ...
                     {isHost && (
                       <button
                         onClick={endBreak}
                         className="group mt-4 flex items-center gap-3 px-8 py-4 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-lg shadow-red-600/20"
                       >
                         Skip
                       </button>
                     )}
                    ...
                    <div className="p-4 space-y-3 pb-8">
                        {teams.map(team => {
                           ...
                           return (
                              <div 
                                key={team.id} 
                                className={`p-4 rounded-xl border transition-all duration-300 cursor-pointer ${isCurrentBidder ? 'border-ipl-gold bg-ipl-gold/10 shadow-[0_0_15px_rgba(209,171,62,0.15)] scale-[1.02]' : 'border-slate-700/50 bg-slate-900/40 hover:bg-slate-800/60 hover:border-slate-600'}`}
