...
export const AuctionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const onBidUpdate = (bid: Bid) => {
      setCurrentBid((prev) => {
        // If we already have this bid (e.g. from optimistic update), don't duplicate logic
        if (prev && prev.id === bid.id) return prev;
        
        // If the incoming bid is lower or equal to current, ignore (race condition protection)
        if (prev && bid.amount <= prev.amount) return prev;
    ...
    const onPlayerSkipped = ({ playerId, timestamp }: { playerId: string; timestamp: number }) => {
      setPlayers(prev => prev.map(p => p.id === playerId ? { ...p, status: 'UNSOLD', timestamp } : p));
      setIsPaused(true);
      setCurrentBid(null);
    };
    ...
    socket.on("playerSkipped", onPlayerSkipped);
    ...
    return () => {
      ...
      socket.off("playerSkipped", onPlayerSkipped);
    };
  }, []);
  ...
  const skipPlayer = () => {
     ...
     const ts = Date.now();
     setPlayers(prev => prev.map(p => p.id === currentPlayerId ? { ...p, status: 'UNSOLD', timestamp: ts } : p));
     setIsPaused(true);
     setCurrentBid(null);
     if (roomCode) {
       socket.emit("skipPlayer", { roomCode, playerId: currentPlayerId, timestamp: ts });
     }
  };
